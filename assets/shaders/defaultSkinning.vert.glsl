...
void main()
{
	...
	vec3 T = normalize(vec3(OBJECT_MATRIX * skinMat * vec4(vTangent, 0.0)));
	vec3 B = normalize(vec3(OBJECT_MATRIX * skinMat * vec4(vBitangent, 0.0)));
	vec3 N = normalize(vec3(OBJECT_MATRIX * skinMat * vec4(vNormal, 0.0)));
	fTBN = mat3(T, B, N);
	...
	fWorldPos = (OBJECT_MATRIX * skinMat * vec4(vPosition, 1.0)).rgb;
	gl_Position = CAMERA_MATRIX * OBJECT_MATRIX * skinMat * vec4(vPosition, 1.0);
}