...
namespace sf
{
	struct ExampleViewer
	{
		Entity gimbal, cameraObject;
		glm::vec3 targetGimbalRotation;
		float cameraDistance;

		void Initialize(Scene& scene, uint32_t cameraRenderTargetId = 0)
		{
			...
			Camera& cameraCamera = cameraObject.AddComponent<Camera>();
			cameraCamera.renderTargetId = cameraRenderTargetId;
		}
		...
		void UpdateCamera(float deltaTime, bool handleInput = true)
		{
			if (handleInput)
		...
		void ImGuiCall(const char* menuName = nullptr)
		{
			if (ImGui::BeginMainMenuBar())
			{
				if (ImGui::BeginMenu(menuName == nullptr ? "Camera" : menuName))
				{
