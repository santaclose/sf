...
namespace errt
{
	...
	void Cylinder(float radius, int steps, const glm::vec3& posA, const glm::vec3& posB, bool cap = true)
	{
		...
		uint32_t quad[4];
		uint32_t* vi = (uint32_t*)alloca(sizeof(uint32_t) * steps * 2);

		uint32_t* capA = (uint32_t*)alloca(sizeof(uint32_t) * steps);
		uint32_t* capB = (uint32_t*)alloca(sizeof(uint32_t) * steps);

		for (int i = 0; i < steps; i++)
		{
			vi[i * 2] = capA[i] = PushVertex(posA + localRight * glm::cos(angleStep * i) * radius + localForward * glm::sin(angleStep * i) * radius);
			vi[i * 2 + 1] = capB[i] = PushVertex(posB + localRight * glm::cos(angleStep * i) * radius + localForward * glm::sin(angleStep * i) * radius);
		...
		PushFace(capA, steps, true);
		PushFace(capB, steps, false);
	}

	void GenerateModel(sf::MeshData& mesh)
	{
		vertices.clear();
		indices.clear();
		...
		srand(seed);
		Cylinder(ERRT_RADIUS, cylinderRes, glm::vec3(0.0f), glm::vec3(0.0f, 5.0f, 0.0f));
		...
		for (int counter = 0; counter < maxCount; counter++)
		{
			...
			*/
			...
			Cylinder(ERRT_RADIUS, cylinderRes, newPosA, newPosB);
