...
void sf::Animation::ComputeNodePose(sf::Animation::Node& node)
{
	...
	case NodeType::Single:
	{
		...
		SampleToBones(*(node.single.animation), node.single.pose, convertedTimer + node.single.animation->start);
		break;
	}
	case NodeType::BlendSpace1D:
	{
		...
		for (int i = 0; i < pointCount; i++)
		{
			...
			SampleToBones(*(node.bs1d.points[i].animation), node.bs1d.points[i].pose, currentPointTimer + node.bs1d.points[i].animation->start);
		}
	...
	case NodeType::BlendSpace2D:
	{
		...
		for (int i = 0; i < pointCount; i++)
		{
			...
			SampleToBones(*(node.bs2d.points[i].animation), node.bs2d.points[i].pose, currentPointTimer + node.bs2d.points[i].animation->start);
		}
