...
sf::VoxelBox::VoxelBox(unsigned int voxelCountX, unsigned int voxelCountY, unsigned int voxelCountZ, float voxelSize, const glm::vec3& offset)
{
...
sf::VoxelBox::VoxelBox(const Mesh& mesh, float voxelSize)
{
	const float RASTERIZE_MAX_DISTANCE = 0.7f;

	this->voxelSize = voxelSize;

	glm::vec3 minP = mesh.vertexVector[0].position;
	glm::vec3 maxP = minP;

	for (int i = 1; i < mesh.vertexVector.size(); i++)
	{
		minP.x = glm::min(minP.x, mesh.vertexVector[i].position.x);
		minP.y = glm::min(minP.y, mesh.vertexVector[i].position.y);
		minP.z = glm::min(minP.z, mesh.vertexVector[i].position.z);
		maxP.x = glm::max(maxP.x, mesh.vertexVector[i].position.x);
		maxP.y = glm::max(maxP.y, mesh.vertexVector[i].position.y);
		maxP.z = glm::max(maxP.z, mesh.vertexVector[i].position.z);
	}
	...
	for (int indexI = 0; indexI < mesh.indexVector.size(); indexI += 3)
	{
		...
		glm::vec3 triNormal = glm::cross(
			mesh.vertexVector[indexB].position - mesh.vertexVector[indexA].position,
			mesh.vertexVector[indexC].position - mesh.vertexVector[indexA].position);

		glm::vec3 trianglebbmin = {
			glm::min(glm::min(mesh.vertexVector[indexA].position.x, mesh.vertexVector[indexB].position.x), mesh.vertexVector[indexC].position.x),
			glm::min(glm::min(mesh.vertexVector[indexA].position.y, mesh.vertexVector[indexB].position.y), mesh.vertexVector[indexC].position.y),
			glm::min(glm::min(mesh.vertexVector[indexA].position.z, mesh.vertexVector[indexB].position.z), mesh.vertexVector[indexC].position.z)
		};
		glm::vec3 trianglebbmax = {
			glm::max(glm::max(mesh.vertexVector[indexA].position.x, mesh.vertexVector[indexB].position.x), mesh.vertexVector[indexC].position.x),
			glm::max(glm::max(mesh.vertexVector[indexA].position.y, mesh.vertexVector[indexB].position.y), mesh.vertexVector[indexC].position.y),
			glm::max(glm::max(mesh.vertexVector[indexA].position.z, mesh.vertexVector[indexB].position.z), mesh.vertexVector[indexC].position.z)
		};
		...
		for (int i = minVoxelCoords.x; i <= maxVoxelCoords.x; i++)
		{
			for (int j = minVoxelCoords.y; j <= maxVoxelCoords.y; j++)
			{
				...
				glm::vec2 tri0 = glm::vec2(mesh.vertexVector[indexA].position.x, mesh.vertexVector[indexA].position.y);
				glm::vec2 tri1 = glm::vec2(mesh.vertexVector[indexB].position.x, mesh.vertexVector[indexB].position.y);
				glm::vec2 tri2 = glm::vec2(mesh.vertexVector[indexC].position.x, mesh.vertexVector[indexC].position.y);
		...
		for (int i = minVoxelCoords.x; i <= maxVoxelCoords.x; i++)
		{
			for (int j = minVoxelCoords.z; j <= maxVoxelCoords.z; j++)
			{
				...
				glm::vec2 tri0 = glm::vec2(mesh.vertexVector[indexA].position.x, mesh.vertexVector[indexA].position.z);
				glm::vec2 tri1 = glm::vec2(mesh.vertexVector[indexB].position.x, mesh.vertexVector[indexB].position.z);
				glm::vec2 tri2 = glm::vec2(mesh.vertexVector[indexC].position.x, mesh.vertexVector[indexC].position.z);
		...
		for (int i = minVoxelCoords.z; i <= maxVoxelCoords.z; i++)
		{
			for (int j = minVoxelCoords.y; j <= maxVoxelCoords.y; j++)
			{
				...
				glm::vec2 tri0 = glm::vec2(mesh.vertexVector[indexA].position.z, mesh.vertexVector[indexA].position.y);
				glm::vec2 tri1 = glm::vec2(mesh.vertexVector[indexB].position.z, mesh.vertexVector[indexB].position.y);
				glm::vec2 tri2 = glm::vec2(mesh.vertexVector[indexC].position.z, mesh.vertexVector[indexC].position.y);
			...
			for (int j = minVoxelCoords.y; j <= maxVoxelCoords.y; j++)
			{
				for (int k = minVoxelCoords.z; k <= maxVoxelCoords.z; k++)
				{
					...
					bool voxelValueForCurrentTri =
						...
						glm::abs(Math::PlanePointDistance(triNormal, mesh.vertexVector[indexA].position, voxelCenter)) < voxelSize * RASTERIZE_MAX_DISTANCE;
...
bool sf::VoxelBox::CastRay(const glm::vec3& origin, const glm::vec3& direction, bool avoidEarlyCollision, float* out_t, bool draw)
{
