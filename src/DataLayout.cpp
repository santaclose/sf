...
// Creates data layout from vertex attributes in shader input file
sf::DataLayout::DataLayout(const char* vertexShaderPath)
{
	...
	FileUtils::ReadFileAsString(std::string(vertexShaderPath) + ".input", fileContents);
	for (int i = 1; i < fileContents.length() && fileContents[i] != '['; i++)
	{
		if (fileContents[i - 1] != '\n')
			continue;
		int firstSpaceIndex = i;
		for (; fileContents[firstSpaceIndex] != ' '; firstSpaceIndex++);
		int secondSpaceIndex = firstSpaceIndex + 1;
		for (; fileContents[secondSpaceIndex] != ' '; secondSpaceIndex++);
		int endOfLineIndex = secondSpaceIndex;
		for (; fileContents[endOfLineIndex] != '\n'; endOfLineIndex++);

		int location = std::stoi(fileContents.substr(i, firstSpaceIndex));
		std::string dataTypeSubstring = fileContents.substr(firstSpaceIndex + 1, secondSpaceIndex - (firstSpaceIndex + 1));
		std::string attributeKindSubstring = fileContents.substr(secondSpaceIndex + 1, endOfLineIndex - (secondSpaceIndex + 1));

		assert(location == this->components.size());
