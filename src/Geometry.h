...
namespace sf::Geometry
{
	...
	inline bool IntersectSphereMesh(const SphereCollider& sphere, const MeshCollider& meshCollider)
	{
		...
		{
			const glm::vec3* a = (glm::vec3*)meshData->AccessVertexComponent(BufferComponent::VertexPosition, meshData->indexVector[j + 0]);
			const glm::vec3* b = (glm::vec3*)meshData->AccessVertexComponent(BufferComponent::VertexPosition, meshData->indexVector[j + 1]);
			const glm::vec3* c = (glm::vec3*)meshData->AccessVertexComponent(BufferComponent::VertexPosition, meshData->indexVector[j + 2]);
