...
void ModelLoader::LoadObjFile(std::vector<Vertex>& vertexVector, std::vector<unsigned int>& indexVector, const std::string& filePath, float scaleFactor, bool smooth)
{
	if (!smooth)
	{
		LoadFacetedObjFile(vertexVector, indexVector, filePath, scaleFactor);
		return;
	}
...
void ModelLoader::LoadGltfFile(std::vector<Vertex>& vertexVector, std::vector<unsigned int>& indexVector, const std::string& filePath)
{
