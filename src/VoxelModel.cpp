...
void sf::VoxelModel::CreateFromModel(const Model& model, float voxelSize)
{
	...
	for (int indexI = 0; indexI < model.m_indexVector.size(); indexI += 3)
	{
		...
		glm::uvec3 minVoxelCoords = {
			(trianglebbmin.x - m_minPos.x) / m_voxelSize,
			(trianglebbmin.y - m_minPos.y) / m_voxelSize,
			(trianglebbmin.z - m_minPos.z) / m_voxelSize,
		};
		glm::uvec3 maxVoxelCoords = {
			(trianglebbmax.x - m_minPos.x) / m_voxelSize,
			(trianglebbmax.y - m_minPos.y) / m_voxelSize,
			(trianglebbmax.z - m_minPos.z) / m_voxelSize,
		};
		...
		for (int i = minVoxelCoords.x; i <= maxVoxelCoords.x; i++)
		{
			for (int j = minVoxelCoords.y; j <= maxVoxelCoords.y; j++)
			{
				...
				xyMat[i - minVoxelCoords.x][j - minVoxelCoords.y] = Math::TriPointDistance2D(voxelCenter, tri0, tri1, tri2) < m_voxelSize / 2.0f;
			}
		}
		...
		for (int i = minVoxelCoords.x; i <= maxVoxelCoords.x; i++)
		{
			for (int j = minVoxelCoords.z; j <= maxVoxelCoords.z; j++)
			{
				...
				xzMat[i - minVoxelCoords.x][j - minVoxelCoords.z] = Math::TriPointDistance2D(voxelCenter, tri0, tri1, tri2) < m_voxelSize / 2.0f;
			}
		}
		...
		for (int i = minVoxelCoords.z; i <= maxVoxelCoords.z; i++)
		{
			for (int j = minVoxelCoords.y; j <= maxVoxelCoords.y; j++)
			{
				...
				zyMat[i - minVoxelCoords.z][j - minVoxelCoords.y] = Math::TriPointDistance2D(voxelCenter, tri0, tri1, tri2) < m_voxelSize / 2.0f;
			}
		}
			...
			for (int j = minVoxelCoords.y; j <= maxVoxelCoords.y; j++)
			{
				for (int k = minVoxelCoords.z; k <= maxVoxelCoords.z; k++)
				{
					...
					bool voxelValueForCurrentTri =
						...
						glm::abs(Math::PlanePointDistance(triNormal, model.m_vertexVector[indexA].position, voxelCenter)) < m_voxelSize / 2.0f;
